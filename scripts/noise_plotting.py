...
ax.errorbar((noise_levels)*100, gru_mean_list, yerr=(gru_low_error, gru_high_error), fmt='.', linewidth=3, markersize=15, label='GRU')
ax.errorbar((noise_levels + 0.001)*100, lstm_mean_list, yerr=(lstm_low_error, lstm_high_error), fmt='.', linewidth=3, markersize=15, label='LSTM')
ax.errorbar((noise_levels + 0.002)*100, esn_mean_list, yerr=(esn_low_error, esn_high_error), fmt='.', linewidth=3, markersize=15, label='ESN')
ax.legend(fontsize=14, loc='center right')
ax.set_xlim([-1, max(noise_levels)*100 * 1.3])
ax.tick_params(axis="both", which="major", labelsize=14)
ax.tick_params(axis="both", which="minor", labelsize=14)
ax.set_ylabel('Error', fontsize=20)
ax.set_xlabel('Noise, $\%\sigma _{train}$', fontsize=20)
ax.set_yscale('log')
