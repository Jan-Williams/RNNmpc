...
lstm_low_error = np.clip(lstm_mean_list - np.quantile(lstm_list, q=0.25, axis=1), 0, 100)
lstm_high_error = np.clip(np.quantile(lstm_list, q=0.75, axis=1) - lstm_mean_list,0,100)
...
ax.errorbar(train_amounts, gru_mean_list, yerr=(gru_low_error, gru_high_error), fmt='.', linewidth=3, markersize=15, label='GRU')
ax.errorbar(train_amounts, lstm_mean_list, yerr=(lstm_low_error, lstm_high_error), fmt='.', linewidth=3, markersize=15, label='LSTM')
ax.errorbar(train_amounts, esn_mean_list, yerr=(esn_low_error, esn_high_error), fmt='.', linewidth=3, markersize=15, label='ESN')
ax.legend(fontsize=14, loc='center right')
ax.set_xlim([300, max(train_amounts * 5)])
ax.tick_params(axis="both", which="major", labelsize=14)
ax.tick_params(axis="both", which="minor", labelsize=14)
ax.set_ylabel('Error', fontsize=20)
ax.set_xlabel('Training Samples', fontsize=20)
ax.set_yscale('log')
ax.set_xscale('log')
