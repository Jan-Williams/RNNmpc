...
class LSTMForecaster(nn.Module):
    """LSTM model for forecasting given known control inputs.

    LSTMForecaster revolves around the use of a pytorch LSTM for advancing the
    ...
    fit(U, S, O, lags, num_epochs, lr, patience, train_split)
        train the LSTM forecaster model
    ...
    def __init__(
        ...
        self.lstm = nn.LSTM(self.Ns + self.Nu, self.Nr, dtype=dtype, batch_first=True)
    ...
    def forward(
        ...
        c_0: torch.DoubleTensor
            initial cell state, dims (1, batch, Nr)
        ...
        _, (hn, _) = self.lstm(input, (r_0, c_0))
        output = hn[0]
    ...
    def fit(
        ...
        with torch.no_grad():
            ...
            c_0 = torch.zeros((1, 1, self.Nr), dtype=torch.float64).to(self.device)
            _, (rn, _) = self.lstm(inputs, (r_0, c_0))
    ...
    def forecast(
        ...
        for step in range(fcast_len - 1):
            ...
            output = self(step_inputs, r_0, c_0).clone()
