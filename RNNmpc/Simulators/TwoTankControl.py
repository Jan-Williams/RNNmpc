...
class TwoTankControl:
    ...
    def __init__(self, model_disc: float = 1.0, control_disc: float = 1.0) -> None:
        """Initialize two tank system with specified control and model discretizations.
        ...
        self.sys = psl.nonautonomous.TwoTank()
        self.sys.ts = model_disc
        self.model_steps = int(control_disc / model_disc)
        self.default_x0 = np.array([[0.3032375], [0.4465485]])
        # Default control distribution should be taken as U(0, 0.4)
    ...
    def simulate(
        ...
        for step in range(t_steps):
            ...
            if step < t_steps - 1:
                ...
                U_rep = np.hstack((U_rep, torch.zeros((2, 1))))
